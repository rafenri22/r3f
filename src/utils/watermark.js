export async function addWatermark(canvas, width = 1920, height = 1080) {
    ...
    // Company name (larger)
    const companyFontSize = Math.floor(width * 0.018) // Responsive font size
    ctx.font = `bold ${companyFontSize}px Inter, sans-serif`
    ctx.fillText('PT. Trijaya Agung Lestari', width / 2, height - 45)
    
    // Developer credit (smaller)
    const devFontSize = Math.floor(width * 0.012)
    ctx.font = `${devFontSize}px Inter, sans-serif`
    ctx.fillStyle = '#e2e8f0'
    ctx.fillText('Developed by Rafky Ferdian Algiffari • Portal TJA', width / 2, height - 25)
